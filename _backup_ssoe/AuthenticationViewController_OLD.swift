...
extension AuthenticationViewController: ASAuthorizationProviderExtensionAuthorizationRequestHandler {
    ...
    public func beginAuthorization(with request: ASAuthorizationProviderExtensionAuthorizationRequest) {
        ...
        logger.log("webloginlog: Request URL: \(request.url.absoluteString)")
     ...
     //  }
        ...
        if let refreshToken = refreshToken {
            Task { @MainActor in
                ...
                if let nonce = try? await self.getNonceFromIdp(clientRequestId: clientRequestId) {
                    ...
                    if let signedToken = signedToken {
                        await MainActor.run {
                            self.startLogin(authURL: authURL, token: signedToken )
                        }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                self.startLogin(authURL: authURL, token: "")
            }
        }
